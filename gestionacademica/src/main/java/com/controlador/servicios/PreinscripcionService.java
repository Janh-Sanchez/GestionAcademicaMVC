...
import com.modelo.persistencia.repositorios.GradoRepositorio;
import com.modelo.persistencia.repositorios.RepositorioGenerico;
...
public class PreinscripcionService {
    ...
    public ResultadoValidacion validarDatosEstudiante(
        ...
        Optional<Grado> gradoOpt = gradoRepositorio.buscarPornombreGrado(nombreGrado.trim());
        if (gradoOpt.isEmpty()) {
    ...
    public Preinscripcion registrarPreinscripcion(
        ...
        try {
            ...
            // 3. Procesar cada estudiante individualmente
            for (Estudiante estudiante : estudiantes) {
                // Validar y obtener Grado persistido
                Grado gradoAspira = estudiante.getGradoAspira();
                if (gradoAspira == null || gradoAspira.getNombreGrado() == null) {
                    throw new IllegalArgumentException("Estudiante no tiene grado asignado");
                }
                
                // Buscar el grado existente
                Optional<Grado> gradoOpt = gradoRepositorio.buscarPornombreGrado(gradoAspira.getNombreGrado());
                if (!gradoOpt.isPresent()) {
                    throw new IllegalArgumentException("Grado no encontrado: " + gradoAspira.getNombreGrado());
                }
                
                // Actualizar estudiante con referencias persistentes
            ...
            // 4. Refrescar la preinscripción para incluir estudiantes
