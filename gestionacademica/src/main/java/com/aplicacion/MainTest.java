...
public class MainTest {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 9. Crear estudiantes PENDIENTES para acudientes pendientes
            List<Estudiante> estudiantesPendientes = crearEstudiantesPendientes(em, acudientesPendientes);
    ...
    private static List<Estudiante> crearEstudiantesPendientes(EntityManager em, List<Acudiente> acudientes) {
        EntityTransaction tx = em.getTransaction();
        ...
        try {
            ...
            System.out.println("Creando estudiantes PENDIENTES (NINGUNO aprobado)...");
            ...
            // Estudiantes para Párvulos (8 estudiantes - TODOS PENDIENTES)
            ...
            // Estudiantes para Caminadores (8 estudiantes - TODOS PENDIENTES)
            ...
            // Estudiantes para Pre-Jardín (8 estudiantes - TODOS PENDIENTES)
            ...
            for (Object[] datos : datosParvulos) {
                ...
                System.out.println("    - " + estudiante.getPrimerNombre() + " " + estudiante.getPrimerApellido() +
                    " (Acudiente: " + acudientes.get(indiceAcudiente).getPrimerNombre() + ")");
                indiceAcudiente = (indiceAcudiente + 1) % acudientes.size();
            }
            
            // Crear estudiantes Caminadores
            System.out.println("\n  Caminadores (TODOS PENDIENTES):");
            for (Object[] datos : datosCaminadores) {
                Estudiante estudiante = crearEstudiante(
                    datos, gradoCaminadores, acudientes.get(indiceAcudiente)
                );
                em.persist(estudiante);
                estudiantes.add(estudiante);
                System.out.println("    - " + estudiante.getPrimerNombre() + " " + estudiante.getPrimerApellido() +
            ...
            System.out.println("\n  Pre-Jardín (TODOS PENDIENTES):");
            for (Object[] datos : datosPreJardin) {
                Estudiante estudiante = crearEstudiante(
                    datos, gradoPreJardin, acudientes.get(indiceAcudiente)
                );
                em.persist(estudiante);
                estudiantes.add(estudiante);
                System.out.println("    - " + estudiante.getPrimerNombre() + " " + estudiante.getPrimerApellido() +
            ...
            tx.commit();
            System.out.println("\n✓ " + estudiantes.size() + " estudiantes PENDIENTES creados (24 total)\n");
            return estudiantes;
        ...
        try {
            ...
            for (Map.Entry<Acudiente, List<Estudiante>> entry : estudiantesPorAcudiente.entrySet()) {
                ...
                System.out.println("  " + contador + ". Preinscripción para " + 
                    ...
                    estudiantesAcudiente.size() + " estudiantes (Estado: PENDIENTE)");
                contador++;
            }
        ...
        try {
            ...
            for (Acudiente acu : acudientesList) {
                ...
                System.out.println("    - " + acu.obtenerNombreCompleto() + 
                    " (" + numEstudiantes + " estudiantes pendientes)");
            }
            
            // Estudiantes por grado
            System.out.println("\n📚 ESTUDIANTES PENDIENTES POR GRADO:");
            String[] grados = {"Párvulos", "Caminadores", "Pre-Jardín"};
            for (String nombreGrado : grados) {
                long count = em.createQuery(
                    "SELECT COUNT(e) FROM Estudiante e WHERE e.gradoAspira.nombreGrado = :grado AND e.estado = :estado", 
                    Long.class)
                    .setParameter("grado", nombreGrado)
                    .setParameter("estado", Estado.Pendiente)
                    .getSingleResult();
                System.out.println("  • " + nombreGrado + ": " + count + " estudiantes");
            }
            ...
            System.out.println("  3. Aprueba o rechaza estudiantes manualmente");
            System.out.println("  4. Observa cómo se generan tokens automáticamente al aprobar");
            System.out.println("  5. Prueba rechazar TODOS los estudiantes de un acudiente");
            System.out.println("     para verificar que el acudiente queda rechazado sin token");
